...
END //
DELIMITER ;
CALL UpdateDataMaster ('ky03', 'moh.safi',@status);
SELECT @status;

-- Menghitung jumlah total entri pada tabel transaksi.
DELIMITER //
...
-- Mendapatkan data dari tabel kategori berdasarkan id
DELIMITER //
CREATE PROCEDURE GetDataMasterByID (
IN id VARCHAR(20),
OUT idKategori VARCHAR(10),
OUT NamaKategori VARCHAR(50)
)
BEGIN
SELECT id_kategori, nama_kategori
INTO idKategori, NamaKategori
FROM kategori
WHERE id_kategori = id;
END //
DELIMITER ;
CALL GetDataMasterByID('k01',@idKategori, @NamaKategori);
SELECT @idKategori, @NamaKategori;

-- Memperbarui dua kolom dalam tabel transaksi.
DELIMITER //
CREATE PROCEDURE UpdateFieldTransaksi(
    IN id VARCHAR(10), 
    INOUT new_total INT, 
    INOUT new_pelanggan VARCHAR(50)
)
BEGIN
    ...
    IF new_pelanggan IS NOT NULL THEN
        UPDATE transaksi
        SET id_pelanggan = new_pelanggan
        WHERE id_transaksi = id;
    END IF;
END //
DELIMITER ;
SET @new_total = 90000;
SET @new_pelanggan = 'c10';
CALL UpdateFieldTransaksi('t09', @new_total, @new_pelanggan);
SELECT  @new_total, @new_pelanggan;

-- Menghapus entri dari tabel kategori berdasarkan id.
DELIMITER //
...
END //
DELIMITER ;
CALL DeleteEntriesByIDMaster('k07');
