CREATE DATABASE db_produk_umkm_modul3;
USE db_produk_umkm_modul3;
...
ALTER TABLE produk_umkm ADD CONSTRAINT fk_id_umkm FOREIGN KEY (id_umkm) REFERENCES umkm(id_umkm)ON DELETE CASCADE;
...
ALTER TABLE umkm ADD CONSTRAINT fk_id_pemilik FOREIGN KEY (id_pemilik) REFERENCES pemilik_umkm(id_pemilik) ON DELETE CASCADE;
ALTER TABLE umkm ADD CONSTRAINT fk_id_kategori FOREIGN KEY (id_kategori ) REFERENCES kategori_umkm(id_kategori) ON DELETE CASCADE;
ALTER TABLE umkm ADD CONSTRAINT fk_id_skala FOREIGN KEY (id_skala) REFERENCES skala_umkm(id_skala) ON DELETE CASCADE;
ALTER TABLE umkm ADD CONSTRAINT fk_id_kabupaten_kota FOREIGN KEY (id_kabupaten_kota ) REFERENCES kabupaten_kota(id_kabupaten_kota)ON DELETE CASCADE;
...
-- SOAL 1 Menambhakan Data UMKM baru ke tabel
DELIMITER //
...
CALL AddUMKM('Warung Bebek Sinjay Sindy', 7);
SELECT * FROM umkm;

DROP PROCEDURE IF EXISTS AddUMKM;

-- SOAL 2 memperbarui nama kategori dalam tabel kategori
DELIMITER //
...
END //
DELIMITER ;
CALL UpdateKategoriUMKM(1, 'Fast Food');
SELECT * FROM kategori_umkm;

-- SOAL 3 menghapus data pemilik UMKM berdasarkan ID 
DELIMITER //
...
-- SOAL 4 menambahkan data produk ke dalam tabel produk_umkm
DELIMITER //
CREATE PROCEDURE AddProduk (
    IN b_id_umkm INT,
    IN b_nama_produk VARCHAR(200),
    IN b_harga DECIMAL(15,2))
BEGIN
    INSERT INTO produk_umkm (id_umkm, nama_produk, harga)
    VALUES (b_id_umkm, b_nama_produk, b_harga);
END //
DELIMITER ;
CALL AddProduk(1, 'Seblak Goreng', 10000.00);
SELECT * FROM produk_umkm;

-- SOAL 5 menerima parameter id_umkm (IN) dan mengembalikan data UMKM
DELIMITER //
...
END //
DELIMITER ;
-- Pertama siapkan variabel OUT-nya
SET @nama := '';
SET @jumlah := 0;
...
DROP DATABASE IF EXISTS db_produk_umkm_modul3;
SHOW DATABASES;