...
id_produk INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
id_pemilik INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
id_kategori INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
nama_kategori VARCHAR(100) NOT NULL,
deskripsi TEXT NOT NULL
);

CREATE TABLE skala_umkm (
id_skala INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
id_kabupaten_kota INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
nama_kabupaten_kota VARCHAR(100) NOT NULL
);


CREATE TABLE umkm (
id_umkm INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
ALTER TABLE produk_umkm ADD CONSTRAINT fk_id_umkm FOREIGN KEY (id_umkm) REFERENCES umkm(id_umkm);

ALTER TABLE umkm ADD CONSTRAINT fk_id_pemilik FOREIGN KEY (id_pemilik) REFERENCES pemilik_umkm(id_pemilik);
ALTER TABLE umkm ADD CONSTRAINT fk_id_kategori FOREIGN KEY (id_kategori) REFERENCES kategori_umkm(id_kategori);
ALTER TABLE umkm ADD CONSTRAINT fk_id_skala FOREIGN KEY (id_skala) REFERENCES skala_umkm(id_skala);
ALTER TABLE umkm ADD CONSTRAINT fk_id_kabupaten_kota FOREIGN KEY (id_kabupaten_kota) REFERENCES kabupaten_kota(id_kabupaten_kota);
...
SELECT u.nama_usaha AS 'Nama Usaha', p.nama_lengkap AS 'Nama Pemilik', k.nama_kategori AS 'kategori Umkm',s.nama_skala AS 'Skala Usaha', kk.nama_kabupaten_kota AS 'Kabupaten/Kota', u.tahun_berdiri AS 'Tahun Brdiri'
FROM umkm u 
...
SELECT p.nik AS 'NIK', p.nama_lengkap AS 'Nama Lengkap', p.jenis_kelamin AS 'Jenis Kelamin', p.nomor_telepon AS 'Nomor Telepon', p.email AS 'Email', u.nama_usaha AS 'Nama Usaha Yang Dimiliki'
FROM pemilik_umkm p 
JOIN umkm u ON p.id_pemilik = u.id_pemilik;
SELECT * FROM view_pemilik_dan_usaha;

CREATE VIEW pemilik_usaha AS SELECT p.nama_lengkap AS 'nama pemilik', u.nama_usaha AS 'usaha yang dimiliki'
FROM pemilik_umkm p JOIN umkm u ON p.id_pemilik=u.id_pemilik;
SELECT * FROM pemilik_usaha;

CREATE VIEW view_produk_umkm AS
SELECT u.nama_usaha AS 'Nama Usaha', p.nama_produk AS 'Nama Produk', p.deskripsi_produk AS 'Deskripsi Produk', p.harga AS 'Harga Produk'
FROM umkm u JOIN produk_umkm p ON u.id_umkm = p.id_umkm;
SELECT * FROM view_produk_umkm;


CREATE VIEW view_umkm_menengah AS
SELECT u.nama_usaha AS 'Nama Usaha', p.nama_lengkap AS 'Nama Pemilik', u.total_aset AS 'Total Asset', u.omzet_per_tahun AS 'Omzet Per Tahun'
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik
WHERE u.id_skala = 3;
