CREATE DATABASE db_produk_umkm;
USE db_produk_umkm;
...
ALTER TABLE pemilik_umkm ADD UNIQUE (nik);
ALTER TABLE pemilik_umkm ADD UNIQUE (email);
ALTER TABLE umkm ADD UNIQUE (nib);
ALTER TABLE umkm ADD UNIQUE (npwp);
...
SELECT
    ...
    k.nama_kategori AS 'Kategori UMKM',
    s.nama_skala AS 'Skala Usaha',
    kk.nama_kabupaten_kota AS 'Nama Kabupaten/Kota',
    u.tahun_berdiri AS 'Tahun Berdiri'
FROM umkm u
...
SELECT
    p.nik AS 'NIK',
    p.nama_lengkap AS 'Nama Pemilik',
    p.jenis_kelamin AS 'Jenis Kelamin',
    p.nomor_telepon AS 'Nomor Telepon',
    p.email AS 'Email',
    u.nama_usaha AS 'Nama Usaha'
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik;
...
SELECT
    ...
    p.deskripsi_produk AS 'Deskripsi Produk',
    p.harga AS 'Harga'
FROM umkm u
JOIN produk_umkm p ON u.id_umkm = p.id_umkm;
SELECT * FROM view_produk_umkm;

-- No 4
CREATE VIEW view_umkm_menengah AS
SELECT
    s.batas_aset_bawah AS 'Skala Usaha Menengah',
    u.nama_usaha AS 'Nama Usaha',
    p.nama_lengkap AS 'Nama Pemilik',
    u.total_aset AS 'Total Aset',
    u.omzet_per_tahun AS 'Omzet Pertahun'
FROM umkm u
JOIN skala_umkm s ON u.id_skala = s.id_skala
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik;
...
SELECT 
    k.nama_kabupaten_kota AS `Nama Kabupaten/Kota`,
    COUNT(u.id_umkm) AS `Jumlah UMKM`
FROM kabupaten_kota k
LEFT JOIN umkm u ON k.id_kabupaten_kota = u.id_kabupaten_kota
GROUP BY k.id_kabupaten_kota, k.nama_kabupaten_kota;
SELECT * FROM view_umkm_per_kota;

DROP DATABASE db_produk_umkm;

SHOW TABLES IN db_produk_umkm;